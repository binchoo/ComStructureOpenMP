...
int main()
{
	...
	max = get_max_serial(unsorted, size);
	
	//print(unsorted, SIZE);
	printf("MAX = %d\n", max);

	countsort(&sorted, unsorted, size, max);
	countsort_serial(&test, unsorted, size, max);
	
	//print(sorted, size);
	//print(test, size);
	if(equalarray(sorted, test, size))
		printf("Serial & Parallel results are equal.\n");
	else
		printf("Serial & Parallel results are NOT equal.\n");
...
int get_max(int* unsorted, int len, int num_threads)
{
	...
	printf("[parallel] get_max with %d threads time : %lf + ", num_threads, omp_get_wtime() - t);
	return max;
}
